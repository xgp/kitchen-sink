...
public class Log {
    ...
    static enum Level {
	DEBUG("DEB"), INFO("INF"), ERROR("ERR");
	private final String abbr;
	Level(String abbr) { this.abbr = abbr; }
	...
	if (level.ordinal() >= systemLevel().ordinal()) {
	    _err(String.format("%s [%s] %s: %s", level.abbr(), dateFormat.get().format(new Date()), category,
	...
	if (providerList.size() == 0) {
	    ...
	    if (providerList.size() > 1) _error(LOG_BOOTSTRAP_CATEGORY, "More than one LogFactory (%d) present. Using %s.", providerList.size(), providerList.get(0));
