...
public class Node {
  ...
  //             public void build(final GroovyObject builder, final Map namespaceMap, final
  // Map<String, String> namespaceTagHints) {
  //                 final Closure c = (Closure) replacementClosure.clone();
  //                 Node.this.replacementNodeStack.pop(); // disable the replacement whilst the
  ...
  //     public void build(final GroovyObject builder, final Map namespaceMap, final Map<String,
  // String> namespaceTagHints) {
  //         if (this.replacementNodeStack.empty()) {
  ...
  //                 final List namespaces = (List) builder.invokeMethod("getNamespaces", new
  // Object[]{});
  ...
  //                     builder.getProperty(getTagFor(this.namespaceURI, current, pending,
  // namespaceMap, namespaceTagHints, newTags, builder));
  //                     builder.invokeMethod(this.name, new Object[]{this.attributes, rest});
  //                 } else {
  ...
  //                             attributesWithNamespaces.put(getTagFor(attributeNamespaceURI,
  // current, pending, namespaceMap, namespaceTagHints, newTags, builder) +
  ...
  //                     builder.getProperty(getTagFor(this.namespaceURI, current, pending,
  // namespaceMap, namespaceTagHints, newTags, builder));
  //                     builder.invokeMethod(this.name, new Object[]{attributesWithNamespaces,
  // rest});
  //                 }
  ...
  //             }
  //         } else {
  //             ((ReplacementNode) this.replacementNodeStack.peek()).build(builder, namespaceMap,
  // namespaceTagHints);
  //         }
  //     }
  ...
  //         String tag = findNamespaceTag(pending, namespaceURI); // look in the namespaces whose
  // declaration has already been emitted
  //         if (tag == null) {
  //             tag = findNamespaceTag(current, namespaceURI);  // look in the namespaces who will
  // be declared at the next element

  //             if (tag == null) {
  //                 // we have to declare the namespace - choose a tag
  //                 tag = findNamespaceTag(local, namespaceURI);  // If the namespace has been
  // decared in the GPath expression use that tag

  //                 if (tag == null || tag.length() == 0) {
  //                     tag = findNamespaceTag(tagHints, namespaceURI);  // If the namespace has
  // been used in the parse documant use that tag
  //                 }

  //                 if (tag == null || tag.length() == 0) { // otherwise make up a new tag and
  // check it has not been used before
  //                     int suffix = 0;

  //                     do {
  //                         final String posibleTag = "tag" + suffix++;

  //                         if (!pending.containsKey(posibleTag) &&
  // !current.containsKey(posibleTag) && !local.containsKey(posibleTag)) {
  //                             tag = posibleTag;
  //                         }
  //                     } while (tag == null);
  //                 }
  ...
  //     private void buildChildren(final GroovyObject builder, final Map namespaceMap, final
  // Map<String, String> namespaceTagHints) {
  //         final Iterator iter = this.children.iterator();
  //         while (iter.hasNext()) {
  //             final Object child = iter.next();
