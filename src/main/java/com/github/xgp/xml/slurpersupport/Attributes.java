...
class Attributes extends NodeChildren {
  ...
  public Iterator childNodes() {
    throw new RuntimeException(
        "Can't get the child nodes on a GPath expression selecting attributes: ...."
            + this.parent.name()
            + "."
            + name()
            + ".childNodes()");
      ...
      protected Object getNextNode(final Iterator iter) {
        while (iter.hasNext()) {
          ...
          if (next instanceof Attribute) {
            ...
            final String value =
                (String) ((Node) next).attributes().get(Attributes.this.attributeName);
            if (value != null) {
