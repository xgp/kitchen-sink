 ...
 * </pre>
 *
 * <p>Note that in some cases, a 'selector' expression may not resolve to a single node. For
 * example:
 *
 * <pre>
 * def rootNode = new XmlSlurper().parseText(
...
public class XmlSlurper extends DefaultHandler {
   ...
   *     <p>If true then whitespace before elements is kept. The default is to discard the
   *     whitespace.
   */
  ...
  /**
   * @return The GPathResult instance created by consuming a stream of SAX events Note if one of the
   *     parse methods has been called then this returns null Note if this is called more than once
   *     all calls after the first will return null
   */
  ...
  /**
   * Parse the content of the specified input stream into an GPathResult Object. Note that using
   * this method will not provide the parser with any URI for which to find DTDs etc
   *
   * @param input
   * @return An object which supports GPath expressions
   * @throws IOException
  ...
  /**
   * Parse the content of the specified reader into a GPathResult Object. Note that using this
   * method will not provide the parser with any URI for which to find DTDs etc
   *
   * @param in
   * @return An object which supports GPath expressions
   * @throws IOException
   ...
   */
  public boolean getFeature(final String uri)
   ...
   */
  public Object getProperty(final String uri)
      throws SAXNotRecognizedException, SAXNotSupportedException {
   ...
   */
  public void setFeature(final String uri, final boolean value)
      throws SAXNotRecognizedException, SAXNotSupportedException {
   ...
   */
  public void setProperty(final String uri, final Object value)
      throws SAXNotRecognizedException, SAXNotSupportedException {
  ...
  public void startElement(
    ...
    if (namespaceURI.length() == 0) {
      ...
      newElement =
          new Node(this.currentNode, localName, attributes, attributeNamespaces, namespaceURI);
    }
