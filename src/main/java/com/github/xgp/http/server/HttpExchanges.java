...
public final class HttpExchanges {
  ...
  private static final ThreadLocal<DateFormat> dateFormatRfc1036 =
      new ThreadLocal<DateFormat>() {
        @Override
        protected DateFormat initialValue() {
          DateFormat df = new SimpleDateFormat("EEEE, dd-MMM-yy HH:mm:ss zzz", Locale.ENGLISH);
          df.setTimeZone(GMT);
          return df;
        }
      };
  ...
  private static final ThreadLocal<DateFormat> dateFormatAsctime =
      new ThreadLocal<DateFormat>() {
        @Override
        protected DateFormat initialValue() {
          DateFormat df = new SimpleDateFormat("EEE MMM d HH:mm:ss yyyy", Locale.ENGLISH);
          df.setTimeZone(GMT);
          return df;
        }
      };
  ...
  /**
   * When thread-local value is not {@code null}, signals that {@link #handle} should abort
   * immediately with an error. This is a hack required because the HttpServer can't handle the
   * Executor rejecting execution of a runnable.
   */
  static final ThreadLocal<Object> abortImmediately = new ThreadLocal<Object>();

  static {
    List<ThreadLocal<DateFormat>> tmpList = new ArrayList<ThreadLocal<DateFormat>>();
  ...
  /** Sends response to GSA. Should only be used when the request method is HEAD. */
  ...
  /**
   * Sends cheaply-generated response message to GSA. This is intended for use with pre-build,
   * canned messages. It automatically handles not sending the actual content when the request
   * method is HEAD. If the content requires a moderate amount of work to produce, then you should
   * manually call {@link #respond} or {@link #respondToHead} depending on the situation.
   */
   ...
   * manually call {@link #respond} or {@link #respondToHead} depending on the situation.
   */
  static void cannedRespond(HttpExchange ex, int code, String template, Object... params)
      throws IOException {
    cannedRespond(ex, code, "text/plain", String.format(template, params));
  }

  private static void cannedRespond(HttpExchange ex, int code, String contentType, String response)
      throws IOException {
  ...
  /**
   * Sends headers and configures {@code ex} for (possibly) sending content. Completing the request
   * is the caller's responsibility.
   */
  static void startResponse(HttpExchange ex, int code, String contentType, boolean hasBody)
      throws IOException {
  ...
  static void respond(HttpExchange ex, int code, String contentType, byte response[])
      throws IOException {
  ...
  /**
   * Redirect client to {@code location}. The client should retrieve the referred location via GET,
   * independent of the method of this request.
   *
   * @param ex exchange
  ...
  /**
   * Parses multi-valued header fields that can also be represented as comma-separated lists.
   * Leading and trailing whitespace is removed from each value. This is the required format for any
   * repeated header, as specified in RFC 2616 Section 4.2.
   *
   * @return list of split and cleaned values, or {@code null} if no such headers were specified
   */
  ...
  /**
   * If the client supports it, set the correct headers and streams to provide GZIPed response data
   * to the client. Because the content may become compressed, users of this method should generally
   * use a {@code responseLength} of {@code 0} when calling {@link
   * HttpExchange#sendResponseHeaders}. The exception is when responding to a HEAD request, in which
   * {@code -1} is required.
   *
   * @param ex exchange
   * @throws IOException ioe
  ...
  /**
   * Retrieves and parses the If-Modified-Since from the request, returning null if there was no
   * such header or there was an error.
   *
   * @param ex exchange
   ...
   *
   * <p>This implementation currently uses an imperfect heuristic, but should work well in most
   * cases. It checks to see if the Date header is present, which is added during {@link
  ...
  /**
   * Parse request GET query parameters of {@code ex} into its parts, correctly taking into account
   * {@code charset}. The encoding of the GET parameters is not specified in the request parameters,
   * so it must be negotiated elsewhere (i.e., via hard-coding). ISO 8859-1 (Latin-1) and UTF-8 are
   * the only commonly used encodings for query parameters.
   *
